...
class FilterOnSaleService
{
    ...
    public function createOnSaleFilterSelect($currentScope, $entityIdsTable)
    {
        ...
        $derivedTable->joinLeft(
            [AdditionalFiltersConfig::ON_SALE_FILTER => $productCollection->getSelect()],
            AdditionalFiltersConfig::ON_SALE_FILTER . '.entity_id  = entities.entity_id',
            [
                'value' => new \Zend_Db_Expr('if('
                    . AdditionalFiltersConfig::ON_SALE_FILTER . '.entity_id is null, 0, 1)'),
            ]
        );
    ...
    public function getOnSaleFilterSelectWithIds($currentScope, $entityIdsTable)
    {
        ...
        $derivedTable->joinLeft(
            ...
            [
                'value'     => new \Zend_Db_Expr('if('
                    . AdditionalFiltersConfig::ON_SALE_FILTER . '.entity_id is null, 0, 1)'),
                'entity_id' => AdditionalFiltersConfig::ON_SALE_FILTER . '.entity_id',
            ]
        );
    ...
    public function addOnSaleToSelect($select, $storeId, $onSaleFilterValue)
    {
        ...
        if ($onSaleFilterValue == 1) {
            ...
            $onSaleFilter            = AdditionalFiltersConfig::ON_SALE_FILTER . '_filter';
            $select->joinRight(
